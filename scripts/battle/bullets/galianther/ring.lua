...
function RingBullet:init(x, y, dir, speed)
    super.init(self, x, y, "bullets/galianther/ring")
...
function RingBullet:onWaveSpawn()
    self.wave.timer:script(function(wait)
        ...
        local bullet = self.wave:spawnBullet("galianther/ring_afterimage", self.init_x, self.init_y, self.physics.direction, self.physics.speed, 0.7)
        bullet:setLayer(self.layer - 0.01)
        table.insert(self.afterimages, bullet)
        wait(2/30)
        if self:isRemoved() then return end
        local bullet = self.wave:spawnBullet("galianther/ring_afterimage", self.init_x, self.init_y, self.physics.direction, self.physics.speed, 0.4)
        bullet:setLayer(self.layer - 0.01)
        table.insert(self.afterimages, bullet)
    end)
end
