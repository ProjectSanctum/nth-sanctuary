...
function Censer:update()
    ...
    if(self.ccon == 0) then
        ...
        self.string_timer = self.string_timer + DTMULT
    ...
    if(self.ccon == 1) then
        ...
        self.acceleration_val = Utils.approach(self.acceleration_val, 0.25, 0.01 * DTMULT)
        self.cur_angle = math.rad(self.hangle) + (math.sin(self.timer * 0.1) * math.rad(self.swing_max))
        self.hang_x = Utils.approach(self.hang_x, Game.battle.arena.x, math.abs(self.hang_x - Game.battle.arena.x) * self.acceleration_val * DTMULT)
        self.hheight = Utils.approach(self.hheight, self.height_goal, math.abs(self.hheight - self.height_goal) * 0.25 * DTMULT)

        self.x = self.hang_x + math.cos(self.cur_angle) * self.hheight

        if not self.did_start then
            self.did_start = true
            self.string_go = true
            Game.battle.timer:lerpVar(self, "y", self.y, 120, 25, -1, "out")
        end

        local spawn_interval = 13 + math.floor(15 * self.ratio)
        local t = math.floor(self.timer)
        local pt = math.floor(self.prev_timer)

        local trigger = false
        if t ~= pt then
            if ((((#enemys > 1 and (t % spawn_interval == 0 or t % spawn_interval == 5)) and (#balthizard > 1 or self.lightup == false)) or (#enemys == 3 and (t % 7) == 0 and self.lightup == true and #balthizard == 1) or (#enemys == 2 and (t % 7) == 0 and self.lightup == true and #balthizard == 1) or (#enemys == 1 and (t % 4) == 0 and self.lightup == true) or (#enemys == 1 and (t % 10) == 0 and self.lightup == false))) then
                if t >= 20 then
                    trigger = true
                end
            end
        ...
        if self.timer >= 150 and not self.did_return then
    ...
    if(self.ccon == 3) then
        self.timer = self.timer + DTMULT
        if self.timer >= 20 and not self.did_end then
            self.did_end = true
            self.line = {0, 0, 0, 0,}
            local attacker
            if(self.wave.same_attack == 2) then
