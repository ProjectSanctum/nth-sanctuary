...
function map:onEnter()
    ...
    Game.world.map.timer:script(function(wait)
        wait(10/30)
        self.con = 1
        Game.world.music:play("vapor_sanct_wip", 0.7, 1)
        ...
        Game.world.timer:tween(1, self.fakefader, {alpha = 1}, nil, function() 
            ...
            self.world:loadMap("vapor_sanctum/vapor_sanctum_1", "spawn")
...
function map:spawnRippleBurst()
    ...
    for i = 0, 8 do
        Game.world.timer:after(seqtime * i, function()
            self:makeVaporRipple()
        end)
    end
end

function map:makeVaporRipple()
    self.world:spawnObject(VaporRipple(love.math.random(0, SCREEN_WIDTH), love.math.random(0, SCREEN_HEIGHT),  self.spawncolor, 60, love.math.random(3, 8), nil, 10), 999999)
end
