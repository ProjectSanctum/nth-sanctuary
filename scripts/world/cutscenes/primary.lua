return {
    ...
    prefall = function (cutscene)
        if Game:hasPartyMember("jamm") then
			...
			cutscene:wait(cutscene:walkToSpeed(jamm, "jpointA3", 8, nil, true))
			cutscene:wait(cutscene:walkToSpeed(jamm, 320, 160, 8))
    ...
    fall = function (cutscene)
		...
		cutscene:detachCamera()
        cutscene:detachFollowers()
		cutscene:panTo(no.x+120, no.y-80, 0.75)
        cutscene:setSpeaker("susie")
        cutscene:text("* (Kris, [wait:5]look! [wait:10]It's Noelle!)", "blush")
		cutscene:walkToSpeed(kris, "kpoint0", 8, nil, true)
        cutscene:walkToSpeed(sus, "spoint", 8)
		sus:setFacing("left")
		cutscene:walkToSpeed(ral, "rpoint0", 10)
		ral:setFacing("left")
		cutscene:wait(1.5)
        no:shake(2,0)
        ...
        cutscene:wait(0.25)
        Game.world.timer:tween(1, no, {y = no.y + 240, x = no.x - 90, rotation = math.rad(-45)}, 'in-circ')
        sus:resetSprite()
		ral:resetSprite()
        cutscene:walkTo(sus, "spoint2", 0.5)
        cutscene:walkTo(ral, "rpoint", 0.5)
        cutscene:walkTo(kris, "kpoint", 0.5)
		kris:setFacing("left")
        ...
        if Game:hasPartyMember("jamm") then
			...
			ral:setFacing("right")
			cutscene:wait(1/2)
			cutscene:wait(cutscene:walkToSpeed(jamm, 600, 480, 0.1))
			cutscene:wait(cutscene:walkToSpeed(jamm, 600, 480, 2))
			cutscene:wait(cutscene:walkToSpeed(jamm, "jpoint2", 2))
			cutscene:wait(1)
			cutscene:setSpeaker(jamm)
