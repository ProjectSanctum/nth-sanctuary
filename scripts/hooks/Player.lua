		...
		if self.falling == 2 then
			...
			for _, event in ipairs(self.world.stage:getObjects(Event)) do
				...
				if self.falseloop then
					if self.falseloopaffects[1] then
						x = MathUtils.wrap(x, self.falseloopx[1], self.falseloopx[2])
					end
					if self.falseloopaffects[2] then
						y = MathUtils.wrap(y, self.falseloopy[1], self.falseloopy[2])
					end
				end
			...
			if self.fallingtimer <= 0 then
				if self.grabon then
					...
					if self.onrotatingtower and self.world.map.cyltower.horizontal then
						if self.onrotatingtower and self.graby > self.world.map.cyltower.tower_circumference then
							self.graby = self.graby - self.world.map.cyltower.tower_circumference
						end
						if self.onrotatingtower and self.graby < 0 then
							self.graby = self.graby + self.world.map.cyltower.tower_circumference
						end
					else
        ...
        if (docharge == 1) then
            ...
            if (self.jumpchargetimer >= (self.charge_times[#self.charge_times] or math.huge)) then
                ...
                if ((self.jumpchargetimer % 8) == 0) then
					...
					if self.onrotatingtower then
						if self.world.map.cyltower.horizontal then
							afterimage.y = self.world.map.cyltower.krisy - self.y
						else
    ...
    for dist = distance, 1, -1 do
        ...
        if allowed then
			...
			for i = 0, dust_amount do
				...
				if self.onrotatingtower then
					if self.world.map.cyltower.horizontal then
						dust_y = self.world.map.cyltower.tower_y
						dust.physics.speed_y = -1
						if self.facing == "down" then
							dust.physics.speed_y = -4*dist-1
				...
				dust.layer = self.layer - 0.01
				if self.onrotatingtower and not self.world.map.cyltower.horizontal then
					dust.physics.speed_y = -1
				end
				self.world:addChild(dust)
			end
			...
			if charged then
				...
				self.climb_during_timer = Game.world.timer:during(duration, function()
					...
					if self.onrotatingtower then
						if self.world.map.cyltower.horizontal then
							afterimage.y = self.world.map.cyltower.krisy
						else
						...
						if self.neutralcon ~= 1 then
							if self.world.map.cyltower.horizontal then
								if self.facing == "down" then
									afterimage.physics.speed_y = -4*dist
								elseif self.facing == "up" then
									afterimage.physics.speed_y = 4*dist
								end
							else
				...
				self.climb_after_2_timer = Game.world.timer:after(duration-clipamount, function ()
					...
					if self.onrotatingtower and self.world.map.cyltower.horizontal then
						if self.onrotatingtower and self.y > self.world.map.cyltower.tower_circumference then
							self.y = self.y - self.world.map.cyltower.tower_circumference
						end
						if self.onrotatingtower and self.y < 0 then
							self.y = self.y + self.world.map.cyltower.tower_circumference
						end
					else
...
function Player:updateClimb()
    ...
    if self.falseloop and not self.physics.move_target then
		...
		if self.falseloopaffects[2] then
			self.y = MathUtils.wrap(self.y, self.falseloopy[1], self.falseloopy[2])
		end
    end
