...
function ClimbWater:draw()
	...
	if Game.world.map.cyltower then
		...
		if cyltower.horizontal then
			if self.watertype == 1 then
				local watcol = ColorUtils.hexToRGB("0EBBE9FF")
				local topy = MathUtils.clamp(yy + self.drawy, self.starty - 10, self.endy - 8)
				local boty = MathUtils.clamp(yy - 40 + (self.scaley * 40) + self.drawy, self.starty, self.endy - 8)
				...
				local tile_angle = MathUtils.lerp(360, 0, (topy + adjustment) / cyltower.tower_circumference)
				local tile_angle1 = tile_angle + cyltower.tower_angle
				...
				if not (tile_angle1 > 350 or tile_angle1 <= 170) then
					...
					local tile_y = MathUtils.lengthDirX(cyltower.tower_radius, -math.rad(tile_angle1))
					local tile_angle2 = tile_angle1 + cyltower.tile_angle_difference
					if tile_angle2 > 360 then
						tile_angle2 = tile_angle2 - 360
					elseif tile_angle2 < 0 then
						tile_angle2 = tile_angle2 + 360
					end
					local tile_yscale = MathUtils.lengthDirX(cyltower.tower_radius, -math.rad(tile_angle2)) - tile_y
					tile_yscale = tile_yscale / cyltower.tile_height_fine
					Draw.draw(self.watertile_top[colindex], xx - 40, cyltower.tower_y + tile_y, 0, 2, -tile_yscale * 2, 0, 8)
					topy = cyltower.tower_y + tile_y
				end
				adjustment = -260
				if cyltower.appearance == 1 then
					adjustment = -520
				end
				tile_angle = MathUtils.lerp(360, 0, (boty + adjustment - 8) / cyltower.tower_circumference)
				tile_angle1 = tile_angle + cyltower.tower_angle
				while tile_angle1 > 360 do
					tile_angle1 = tile_angle1 - 360
				end
				if tile_angle1 < 0 then
				...
				if not (tile_angle1 > 350 or tile_angle1 <= 170) then
					...
					if not self.ending then
						Draw.draw(self.watertile_top[colindex], xx - 40, cyltower.tower_y + tile_y - 8, 0, 2, tile_yscale * 2, 0, 8)
					end
					boty = cyltower.tower_y + tile_y
				end
				Draw.setColor(ColorUtils.mergeColor({0,0,0,1}, watcol, alph))
				topy = math.max(topy, (cyltower.tower_y - cyltower.tower_radius))
				if ((boty - topy) + offset) > 0 then
					Draw.draw(Assets.getTexture("bubbles/fill"), xx - 40, topy, 0, 40, (boty - topy) + offset)
				end
