local HorizontalTowerReticleDrawer, super = Class(Object)

function HorizontalTowerReticleDrawer:init(cyltower)
    super.init(self)
	self.x, self.y = 0, 0
	self.cyltower = cyltower
end

function HorizontalTowerReticleDrawer:draw()
    ...
    if (player.jumpchargecon ~= 0) then
		...
		if tiley >= cyltower.verticaltilecount then
			tiley = tilex - cyltower.verticaltilecount
		end
		if tiley < 0 then
			tiley = tiley + cyltower.verticaltilecount
		end
		...
		local tile = cyltower.tile_data[cyltower.tm_tileset[1]][math.floor(tiley) + 1]
		local startx = px + xoff + 20
		local starty = (cyltower.tower_y + tile.y + yoff) - 20
		local divisor = 120
        ...
        for subsection = 0, count do
			...
			if ty >= cyltower.verticaltilecount then
				ty = ty - cyltower.verticaltilecount
			end
			if ty < 0 then
				ty = ty + cyltower.verticaltilecount
			end
			local tile2 = cyltower.tile_data[cyltower.tm_tileset[1]][math.floor(ty) + 1]
			if tile2.vis == 1 then
				local jankfix = 0
				if subsection == (#player.charge_times - 1) and cyltower.tile_height_fine ~= cyltower.tile_height and shifty == -1 then
					jankfix = (6 * (shifty - 1)) / 2
				end
				local scalemult = tile2.yscale / cyltower.tile_height_fine
				local quad = Assets.getQuad(0, 0, 22, math.floor(MathUtils.clamp(w - subsection, 0, 1) * h), 22, h)
				Draw.draw(Assets.getFrames(id)[frame], quad, startx, starty - jankfix, -math.rad(angle), 2, scalemult * -2)
				startx = startx + (shiftx * h * 2)
				starty = starty + (scalemult * shifty * h * -2)
			end
		end
    end
    ...
    if (player.jumpchargecon > 0 and found ~= 0) then
		...
		local tile = cyltower.tile_data[cyltower.tm_tileset[1]][math.floor(tiley) + 1]
		if tile.vis == 1 then
			Draw.draw(Assets.getTexture("ui/climb/reticle"), px, cyltower.tower_y + tile.y, 0, 2, (tile.yscale / cyltower.tile_width_fine) * 2, 2, 2)
		end
	end
    love.graphics.pop()
...
return HorizontalTowerReticleDrawer
