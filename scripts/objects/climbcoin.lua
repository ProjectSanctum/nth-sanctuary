...
function ClimbCoin:update()
    ...
    if self.con == 1 then
        ...
        Game.world.timer:after(delaytime + 1/30, function()
            Assets.playSound("leaf_dodge", vol - 0.2, 1.3)
        end)
        Game.world.timer:after((delaytime * 2) + 5/30, function()
            Assets.playSound("leaf_dodge", vol - 0.4, 1.1)
        end)
        Game.world.timer:after((delaytime * 3) + 7/30, function()
            Assets.playSound("leaf_dodge", vol - 0.6, 0.9)
        end)
...
function ClimbCoin:draw()
    local sinamt = math.sin(self.siner / 20) * 6 * math.max(0, math.min(1, 1 - (self.bowlindex / 7)))
