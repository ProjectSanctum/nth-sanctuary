...
function BalthizardActorSprite:init(actor)
    ...
    self.transition_sound = false
    self.transition_hit = false
    self.transition_end_sound = false
end
...
function BalthizardActorSprite:update()
    ...
    if anim == "transition" then
        ...
        if self.transitioncon == 1 then
            self.transitiontimer = self.transitiontimer + DTMULT
            if self.transitiontimer >= 1 and not self.transition_sound then
                self.transition_sound = true
                Assets.playSound("equip")
            end
            local t = math.min(self.transitiontimer / 5, 1)
            self.tail.x = MathUtils.lerp(0, -30, t)
            if self.transitiontimer < 6 then
                self.body.alpha = 0
                self.body_transition.alpha = 1
                self.body_transition:setFrame(math.floor(self.bodyindex))
                self.bodyindex = self.bodyindex + DTMULT
            end
            self.headfire.alpha = 0
            self.leg1.alpha = 0
            self.leg2.alpha = 0
            ...
            self.leg1index = self.leg1index + 0.5 * DTMULT
            self.head.y = self.head.y + 1 * DTMULT
            self.head.scale_x = MathUtils.lerp(1, 0.8, t)
            self.head.scale_y = MathUtils.lerp(1, 0.8, t)
            self.leg2_transition.x = MathUtils.lerp(0, 10, t)
            self.leg3_transition.x = MathUtils.lerp(0, 10, t)
            self.leg2_transition.y = self.leg2_transition.y - 1 * DTMULT
            self.leg3_transition.y = self.leg3_transition.y - 1 * DTMULT
        ...
        if self.transitioncon == 2 then
            self.transitiontimer = self.transitiontimer + DTMULT
            local t = math.min(self.transitiontimer / 5, 1)
            self.head.alpha = MathUtils.lerp(1, 0, t)
            self.tail.alpha = MathUtils.lerp(1, 0, t)
            self.leg1_transition.alpha = MathUtils.lerp(1, 0, t)
            self.leg2_transition.alpha = MathUtils.lerp(1, 0, t)
            self.leg3_transition.alpha = MathUtils.lerp(1, 0, t)
            self.leg1_transition.x = MathUtils.lerp(0, 20, t)
            self.leg2_transition.x = MathUtils.lerp(10, -10, t)
            self.leg3_transition.x = MathUtils.lerp(10, -15, t)
            self.head.x = MathUtils.lerp(self.head.x, 20, t)
            if self.transitiontimer >= 3 and not self.transition_hit then
                self.transition_hit = true
    ...
    if anim == "transition_end" then
        ...
        if self.transitioncon == 6 then
            self.transitiontimer = self.transitiontimer + DTMULT
            if self.transitiontimer >= 1 and not self.transition_end_sound then
                self.transition_end_sound = true
                Assets.playSound("equip")
            end
            local t = math.min(self.transitiontimer / 5, 1)
            self.head.alpha = MathUtils.lerp(0, 1, t)
            self.tail.alpha = MathUtils.lerp(0, 1, t)
            self.leg1_transition.alpha = MathUtils.lerp(0, 1, t)
            self.leg2_transition.alpha = MathUtils.lerp(0, 1, t)
            self.leg3_transition.alpha = MathUtils.lerp(0, 1, t)
            self.leg1_transition.x = MathUtils.lerp(20, 0, t)
            self.leg2_transition.x = MathUtils.lerp(-10, 10, t)
            self.leg3_transition.x = MathUtils.lerp(-15, 10, t)
            self.head.x = MathUtils.lerp(self.head.x, self.head_last_x, t)
        ...
        if self.transitioncon == 7 then
            self.transitiontimer = self.transitiontimer + DTMULT
            local t = math.min(self.transitiontimer / 5, 1)
            self.tail.x = MathUtils.lerp(-30, 0, t)
            ...
            self.leg1index = self.leg1index - 0.5 * DTMULT
            self.head.y = self.head.y - 1 * DTMULT
            self.head.scale_x = MathUtils.lerp(0.8, 1, t)
            self.head.scale_y = MathUtils.lerp(0.8, 1, t)
            self.leg2_transition.x = MathUtils.lerp(10, 0, t)
            self.leg3_transition.x = MathUtils.lerp(10, 0, t)
            self.leg2_transition.y = self.leg2_transition.y + 1 * DTMULT
            self.leg3_transition.y = self.leg3_transition.y + 1 * DTMULT
            if self.transitiontimer >= 5 then
                self.transitiontimer = 0
