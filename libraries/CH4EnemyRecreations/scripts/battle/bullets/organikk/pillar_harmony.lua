...
function PillarHarmony:update()
    ...
    if self.wave.del then
        if self.charge_sfx then self.charge_sfx:stop(); self.charge_sfx = nil end
        if self.highlight then self.highlight:remove(); self.highlight = nil end
    end
    ...
    if self.touching_player > 0 then
        if not self.charge_sfx then
            ...
            Game.battle.timer:during(1/2, function()
                ...
                if self.charge_sfx then
                    self.charge_sfx:setVolume(MathUtils.rangeMap(sfx_timer, 0, 1/2, 0, 1))
                end
            end)
        ...
        if self.particle_timer >= 3 then
            for _, attacker in ipairs(self.wave:getAttackers()) do
                local p1 = Game.battle:addChild(OrganikkParticle(attacker.x - 15 + MathUtils.random(-20, 20), attacker.y - 20))
                p1:setLayer(BATTLE_LAYERS["above_battlers"])
                local p2 = Game.battle:addChild(OrganikkParticle(Game.battle.soul.x - 12 + MathUtils.random(-5, 5), Game.battle.soul.y - 4))
                p2:setLayer(BATTLE_LAYERS["above_soul"])
            end
            self.particle_timer = self.particle_timer - 3
        end

        self.gainmercytimer = self.gainmercytimer + (DT * 60)
        if self.gainmercytimer >= 2 then
            for _, attacker in ipairs(self.wave:getAttackers()) do
                attacker.mercyget = 1
            end
            self.gainmercytimer = self.gainmercytimer - 2
        end

        self.gainmercytimer2 = self.gainmercytimer2 + (DT * 60)
        if self.gainmercytimer2 >= 4 then
            for _, attacker in ipairs(self.wave:getAttackers()) do
                attacker.mercyget2 = 1
            end
            self.gainmercytimer2 = self.gainmercytimer2 - 4
        end
